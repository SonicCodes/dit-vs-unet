...
def main(load_dir, save_dir, fid_stats, seed, log_interval, eval_interval, save_interval,
         batch_size, max_steps, model_type, model_size, denoise_timesteps, _cfg_scale ):
    ...
    wandb_config.update({
        'project': 'flow',
        'name': f'{model_type}_{model_size}'
    })
    ...
    # Data loading
    dataset = get_dataset(batch_size=batch_size, train=True)
    dataset_valid = get_dataset(batch_size=batch_size, train=False)
